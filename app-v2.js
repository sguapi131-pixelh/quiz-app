(() => {
  ...
  function loadBuiltInQuestions() {
    ...
    if (raw.length !== 750) throw new Error(`题库数量异常：${raw.length}（应为 750）`);
    const ids = new Set();
    const out = raw.map((row, idx) => {
    ...
    const single = out.filter(q => q.type === "single").length;
    const multiple = out.filter(q => q.type === "multiple").length;
    if (single !== 354 || multiple !== 396) throw new Error(`题型数量异常：单选 ${single} / 多选 ${multiple}`);
    return out;
  }
